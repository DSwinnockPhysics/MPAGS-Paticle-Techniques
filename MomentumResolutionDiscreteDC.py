...
nEventsSkipped = 0
nEventsMore5Hits = 0
DiscretizeDriftChambers = True

for i in range(nevents):
    # First fit the line of best fit
    if DiscretizeDriftChambers == True:
        CurrentDc1HitsXRounded = []
        CurrentDc2HitsXRounded = []
        for j in Dc1HitsX[i]:
            CurrentDc1HitsXRounded.append(round(j,4)) # Apply 100um precision
        for j in Dc2HitsX[i]:
            CurrentDc2HitsXRounded.append(round(j,4))
        FitCoeffsDc1[i] = FindBestFitLine(numpy.array(Dc1HitsZ[i]),numpy.array(CurrentDc1HitsXRounded))
        FitCoeffsDc2[i] = FindBestFitLine(numpy.array(Dc2HitsZ[i]),numpy.array(CurrentDc2HitsXRounded))
    else:
    ...
    # The perpendicular method doesn't work after the x coordinates are discretized as the gradients become too small,
    # resulting in gradients so large they cause overflows.
